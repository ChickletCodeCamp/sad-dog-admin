...
export class CookieDao implements Dao {
   ...
   */
  create<T>(key: string, value: T, time: number = 6000): void {
    // 序列化物件
    let data: string = JSON.stringify(value);
    var date: Date = new Date();
    date.setTime(date.getTime() + time * 1000); //100 minute
  ...
  read<T>(key: string): T | null {
    ...
    for (var i = 0; i < ca.length; i++) {
      ...
      if (c.indexOf(nameEQ) === 0) {
        let req: string = decodeURIComponent(
   ...
   * @param time 過期時間 (秒)(非必填)
   */
  update<T>(key: string, value: T, time: number = 6000): void {
    // 序列化物件
    let data: string = JSON.stringify(value);
    var date: Date = new Date();
    date.setTime(date.getTime() + time * 1000); //100 minute
